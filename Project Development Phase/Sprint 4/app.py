...
content = Content("text/plain","Items below threshold")
...
def view_stocks():
	...
	location=[]
	if request.method == 'GET':
		sql="SELECT DISTINCT PRODUCT_ID FROM VVX91242.PRODUCT_DETAILS"
		stmt= ibm_db.exec_immediate(conn,sql)
		while ibm_db.fetch_row(stmt) != False:
			product_ID.append(ibm_db.result(stmt,0))

		sql="SELECT DISTINCT PRODUCT_NAME FROM VVX91242.PRODUCT_DETAILS"
		stmt= ibm_db.exec_immediate(conn,sql)
		while ibm_db.fetch_row(stmt) != False:
			product_name.append(ibm_db.result(stmt,0))
	
		sql="SELECT DISTINCT SUPPLIER FROM VVX91242.PRODUCT_DETAILS"
		stmt= ibm_db.exec_immediate(conn,sql)
		while ibm_db.fetch_row(stmt) != False:
			supplier.append(ibm_db.result(stmt,0))

		sql="SELECT DISTINCT LOCATION FROM VVX91242.PRODUCT_ADD_DETAILS"
		stmt= ibm_db.exec_immediate(conn,sql)
		while ibm_db.fetch_row(stmt) != False:
	...
	if request.method == 'POST':
		...
		location = request.form['location']
		if product_ID!="None":
			sql="SELECT DISTINCT VVX91242.PRODUCT_DETAILS.PRODUCT_ID,VVX91242.PRODUCT_DETAILS.PRODUCT_NAME,VVX91242.PRODUCT_DETAILS.QUANTITY,SUPPLIER FROM VVX91242.PRODUCT_DETAILS JOIN PRODUCT_ADD_DETAILS ON VVX91242.PRODUCT_DETAILS.PRODUCT_ID=VVX91242.PRODUCT_ADD_DETAILS.PRODUCT_ID WHERE VVX91242.PRODUCT_DETAILS.PRODUCT_ID="+product_ID
		else:
			sql="SELECT DISTINCT VVX91242.PRODUCT_DETAILS.PRODUCT_ID,VVX91242.PRODUCT_DETAILS.PRODUCT_NAME,VVX91242.PRODUCT_DETAILS.QUANTITY,SUPPLIER FROM VVX91242.PRODUCT_DETAILS JOIN VVX91242.PRODUCT_ADD_DETAILS ON VVX91242.PRODUCT_DETAILS.PRODUCT_ID=VVX91242.PRODUCT_ADD_DETAILS.PRODUCT_ID "
		if product_name!="None":
			if "WHERE" in sql:
				sql+=" AND VVX91242.PRODUCT_DETAILS.PRODUCT_NAME='"+product_name+"'"
			else:
				sql+="WHERE VVX91242.PRODUCT_DETAILS.PRODUCT_NAME='"+product_name+"'"
		if quantity!="None":
			if "WHERE" in sql:
				if qty_1=="lesser than":
					sql+=" AND VVX91242.PRODUCT_DETAILS.QUANTITY<"+quantity
				elif qty_1=="greater than":
					sql+=" AND VVX91242.PRODUCT_DETAILS.QUANTITY>"+quantity
			else:
				if qty_1=="lesser than":
					sql+="WHERE VVX91242.PRODUCT_DETAILS.QUANTITY<"+quantity
				elif qty_1=="greater than":
					print("hi")
					sql+="WHERE VVX91242.PRODUCT_DETAILS.QUANTITY>"+quantity
		if threshold!="None":
			if "WHERE" in sql:
				if threshold=="lesser than":
					sql+=" AND VVX91242.PRODUCT_DETAILS.QUANTITY<THRESHOLD_QTY"
				elif threshold=="greater than":
					sql+=" AND VVX91242.PRODUCT_DETAILS.QUANTITY>THRESHOLD_QTY"
			else:
				if threshold=="lesser than":
					sql+="WHERE VVX91242.PRODUCT_DETAILS.QUANTITY<THRESHOLD_QTY"
				elif threshold=="greater than":
					sql+="WHERE VVX91242.PRODUCT_DETAILS.QUANTITY>THRESHOLD_QTY"
		if supplier!="None":
			if "WHERE" in sql:
